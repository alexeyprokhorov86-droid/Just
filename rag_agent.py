...
def extract_named_entities(query: str) -> list:
    """Извлекает имена компаний, людей и ID из запроса для обязательного включения в keywords.
    Возвращает токены в нижнем регистре."""
    q = query or ""
    entities = []
    # Русские слова в ВЕРХНЕМ регистре длиннее 3 букв (ДИКСИ, АГРОТОРГ, ИТЕКО, МАГНИТ)
    for m in re.finditer(r"\b[А-ЯЁ]{3,}\b", q):
        entities.append(m.group().lower())
    # ИО Фамилия (кириллица с большой буквы, минимум 4 буквы)
    for m in re.finditer(r"\b[А-ЯЁ][а-яё]{3,}\b", q):
        word = m.group().lower()
        if word not in SEARCH_STOP_WORDS:
            entities.append(word)
    # Числа и коды (номера заказов, артикулы: 1189, 45631724)
...
def search_telegram_chats_sql(query: str, limit: int = 30, target_tables: list = None,
    ...
    named = extract_named_entities(query)
    keyword_terms = []
    seen_kw = set()
    for kw in named + keywords:
...
def search_emails_sql(query: str, limit: int = 30, time_context: dict = None) -> list:
    ...
    retrieval_query = expand_query_for_retrieval(query)
    keywords = select_search_keywords(retrieval_query, max_keywords=8)
    # Сущности из запроса (компании, люди, номера) — всегда в начале, чтобы не вытеснились
    named = extract_named_entities(query)
    keyword_terms = []
    seen_kw = set()
    for kw in named + keywords:
    ...
    try:
        with conn.cursor() as cur:
            ...
            # Приоритетный ILIKE-поиск по именам сущностей (компании, люди, номера).
            # Годы (4-цифры вида 20XX) пропускаем — слишком широкий матч.
            # Они получают lexical_score=0.85 и всегда окажутся выше generic FTS-результатов.
            entity_rows: list = []
            if named:
                for entity in named[:4]:  # max 4 сущности
                    if len(entity) < 3:
                        continue
                    if re.fullmatch(r"20\d{2}", entity):  # год типа 2026
                        continue
                    ent_sql = """
                        SELECT id, subject, body_text, from_address, received_at
                        FROM email_messages
                        WHERE (subject ILIKE %s OR body_text ILIKE %s OR from_address ILIKE %s)
                    """
                    ent_params = [f"%{entity}%", f"%{entity}%", f"%{entity}%"]
                    if date_from:
                        ent_sql += " AND received_at >= %s"
                        ent_params.append(date_from)
                    if date_to:
                        ent_sql += " AND received_at <= %s"
                        ent_params.append(date_to)
                    ent_sql += f" ORDER BY received_at {sort_order} LIMIT %s"
                    ent_params.append(limit)
                    cur.execute(ent_sql, ent_params)
            ...
            # Объединяем: сначала entity hits (score=0.85), потом FTS/ILIKE (score=0.62)
            seen_ids: set = set()
            combined_rows: list = []
            for row in entity_rows:
                if row[0] not in seen_ids:
                    seen_ids.add(row[0])
                    combined_rows.append((row, 0.85))
            for row in fts_results:
                if row[0] not in seen_ids:
                    seen_ids.add(row[0])
                    combined_rows.append((row, 0.62))

            for row, lexical_score in combined_rows:
...
mart_production (производство):
  id, doc_date, doc_number, department_key, nomenclature_key, nomenclature_name,
  quantity, price, sum_total, year, month, year_month
  → nomenclature_name уже денормализована, JOIN к nomenclature не нужен
...
c1_bank_balances: id, period DATE, bank_account_key UUID, opening_balance, incoming, outgoing, closing_balance
c1_bank_accounts: id, ref_key UUID, org_name, bank_name, account_number
  JOIN: c1_bank_accounts ON ref_key::text = c1_bank_balances.bank_account_key::text
  → org_name ('Фрумелад'/'НФ'), bank_name ('Т-Банк', 'Металлинвестбанк' и т.п.)
...
def apply_relevance_filters(results: list) -> list:
    ...
    for r in results:
        ...
        if bucket == "chat":
            ...
            min_score = EMAIL_VECTOR_MIN_SCORE if "vector" in search_type else EMAIL_SQL_MIN_SCORE
            if score < min_score:
                continue
...
"""
    ...
    try:
        ...
        logger.warning("Evaluator timeout — treating as sufficient")
        return {"sufficient": True, "missing": "", "retry_keywords": "", "retry_chats": []}
    except Exception as e:
...
async def process_rag_query(question, chat_context="", user_info: dict = None,
    ...
    if meta_out is not None:
        ...
        meta_out["evidence_count"] = len(evidence_results)
        meta_out["answer_eval_good"] = answer_eval.get("good") if answer_eval else None
        meta_out["router_query_type"] = plan.get("query_type", "")
        meta_out["steps"] = executed_sources
