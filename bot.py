...
async def download_and_analyze_media(bot, message, table_name: str = None) -> tuple[str, str, str]:
    ...
    try:
        ...
        if message.photo:
            ...
            # Видео любого размера качаем через MTProto user-client (до 2 GB).
            # См. tools.attachments.analyze_video_from_telegram.
            media_type = "video"
            media_type_str = "video"
            filename = "video.mp4"
            # file не нужен — tool сам качает по (chat_id, message_id).
        elif message.document:
            doc = message.document
            filename = doc.file_name or ""
            filename_lower = filename.lower()

            # Видео как документ — тот же путь через MTProto.
        ...
        # Видео качается через MTProto в tool'е (file остаётся None).
        # Для всех остальных типов — качаем через Bot API сейчас.
        ...
        if media_type == "voice" or media_type == "audio":
            ...
            # MTProto-путь: tool сам скачивает видео (до 2 GB), делает adaptive
            # frame sampling (scene-det + LLM-classify + optional deep-scan),
            # Whisper-транскрипт и summary.
            from tools.attachments import analyze_video_from_telegram
            result = await analyze_video_from_telegram(
            ...
            content_text = result.get("extracted_text", "")
            if result.get("errors"):
                logger.warning(f"video analysis warnings: {result['errors']}")
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.chat.type == "private":
        ...
        await update.message.reply_text(
            ...
            "Просто напиши вопрос — отвечу через поиск по 1С/чатам/email.\n\n"
            "<b>Часто используемые:</b>\n"
            "/search &lt;запрос&gt; — поиск по базе знаний\n"
            "/tasks — мои открытые задачи (платежи, заказы)\n"
            "/bom — состав продукции и калькуляции (для группы «Новые продукты и конкуренты»)\n"
            "/fix — задача на исправление в Claude Code (для Руководства)\n"
            "/element / /rooms — данные и приглашения Element X\n\n"
            "<b>Email:</b>\n"
            "/threads — открытые цепочки писем\n"
            "/search_email &lt;запрос&gt; — поиск по email\n"
            "/email_stats — статистика входящих\n\n"
            "<b>В групповых чатах:</b>\n"
            "/roles — пользователи без ролей\n"
            "/stats — статистика чата\n"
            "/analysis — настройка рассылки анализа документов\n\n"
            "<b>Уведомления:</b>\n"
            "/notify — отправить рассылку (для админа)\n"
            "/notify_status — статус подтверждений\n"
            "/notify_remind — повторно напомнить",
            parse_mode='HTML'
        )
    else:
        await update.message.reply_text(
...
async def backfill_videos_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Admin-команда: пройтись по tg_chat_* видео и прогнать через
    analyze_video_from_telegram.

    Без аргументов — только пустые media_analysis.
    `/backfill_videos all` — ВСЕ видео, даже уже с анализом (перезапись,
    например после изменения промпта/модели).
    """
    if update.effective_user.id != ADMIN_USER_ID:
        await update.message.reply_text("⛔ Только для администратора")
        return
    ...
    reprocess_all = bool(args) and args[0].lower() in ("all", "reprocess", "force")

    candidates = []
    conn = get_db_connection()
    try:
        with conn.cursor() as cur:
            cur.execute("SELECT table_name, chat_title, chat_id FROM tg_chats_metadata WHERE table_name IS NOT NULL")
            mapping = {r[0]: (r[1], r[2]) for r in cur.fetchall()}
            cur.execute("SELECT tablename FROM pg_tables WHERE tablename ~ '^tg_chat_\\d'")
            tables = [r[0] for r in cur.fetchall()]
        filter_clause = "" if reprocess_all else " AND (media_analysis IS NULL OR media_analysis = '')"
        for t in tables:
            title, chat_id = mapping.get(t, ("?", None))
            if chat_id is None:
                continue
            with conn.cursor() as cur:
                cur.execute(
                    f'SELECT message_id FROM "{t}" WHERE message_type = \'video\'{filter_clause} ORDER BY created_at',
                )
                for (mid,) in cur.fetchall():
                    candidates.append({"table": t, "chat_title": title, "chat_id": chat_id, "message_id": mid})
    finally:
        conn.close()

    if not candidates:
        await update.message.reply_text("✅ Нет видео для обработки.")
        return

    mode_label = "ВСЕ (перезапись)" if reprocess_all else "только пустые"
    await update.message.reply_text(
        f"🎬 Backfill {len(candidates)} видео ({mode_label}). Фоном, ~{max(3, len(candidates)*4)} мин. Отчёт пришлю в личку."
    )

    async def _run():
        from tools.attachments import analyze_video_from_telegram
        report = [f"🎬 Backfill результаты ({len(candidates)} шт.):"]
        ok, fail = 0, 0
        total_bytes, total_elapsed = 0, 0.0

        for i, item in enumerate(candidates, 1):
            label = f"[{i}/{len(candidates)}] {item['chat_title']} / msg={item['message_id']}"
            logger.info(f"backfill: {label}")
            t0 = time.time()
            try:
                result = await analyze_video_from_telegram(
                    chat_id=item["chat_id"],
                    message_id=item["message_id"],
                    filename=f"video_{item['message_id']}.mp4",
                )
            except Exception as e:
                fail += 1
            ...
            transcript = result.get("extracted_text") or ""
            errors = result.get("errors") or []

            if not summary and errors:
                fail += 1
                report.append(f"❌ {label}: {'; '.join(errors)[:150]}")
                continue
            ...
            try:
                ...
                with c2.cursor() as cur:
                    cur.execute(
                        f'UPDATE "{item["table"]}" SET media_analysis = %s, content_text = %s WHERE message_id = %s',
                        (summary, transcript, item["message_id"]),
                    )
                    c2.commit()
                c2.close()
                if summary and len(summary) > 50:
                    try:
                        await extract_and_save_facts(summary, source=f"document:video_{item['message_id']}.mp4")
                    except Exception as fe:
                        logger.debug(f"fact extract warn: {fe}")
            except Exception as db_e:
                fail += 1
                report.append(f"❌ {label}: DB update failed: {db_e}")
                continue

            ok += 1
            sf = result.get("structured_fields") or {}
            total_bytes += sf.get("downloaded_bytes", 0)
            total_elapsed += elapsed
            report.append(
                f"✅ {label}: {sf.get('duration_sec', 0)/60:.1f}m, {sf.get('visual_density', '?')}, "
                f"{sf.get('final_frames_in_summary', 0)}фр, summary={len(summary)}ch, {elapsed:.0f}s"
            )

        report.append("")
        report.append(f"Итого: ✅ {ok}, ❌ {fail}")
        report.append(f"Объём: {total_bytes/1024/1024:.0f} MB, время {total_elapsed/60:.1f} мин")

        # Отправляем отчёт админу (по частям если длинно)
        full = "\n".join(report)
        for chunk_start in range(0, len(full), 3800):
            try:
                await context.bot.send_message(
                    chat_id=ADMIN_USER_ID,
                    text=full[chunk_start:chunk_start + 3800],
                    disable_web_page_preview=True,
                )
            except Exception as se:
                logger.error(f"backfill send_message failed: {se}")
                break
...
def main():
    ...
    application.add_handler(CommandHandler("backfill_videos", backfill_videos_command))
